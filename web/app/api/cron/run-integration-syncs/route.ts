...
// Drives the integration sync engine: re-runs connected-workbook syncs
// that failed transiently and are now due for a retry, and fails runs
// that got stuck in `running` past the lease. Authenticated by the
// shared cron secret, never a browser session; excluded from the app
// middleware like every /api/cron/* route.
//

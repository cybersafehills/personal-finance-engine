-- Integrations Phase 3, P3-PR7: widen the operator health snapshot's
-- `integrations` block with accountant-package and connected-ledger
-- aggregates. Forward-only replace of the wrapper only
-- (get_operational_health_snapshot_core is unchanged). Still
-- identifier-free / service-role only - no tenant ids, no financial
-- values, no provider names.
...
comment on function public.get_operational_health_snapshot(integer) is
  'Service-only aggregate health snapshot: ingestion, duplicate review, report jobs, email, payment reconciliation, and Integrations import/export/sync/conflicts/accountant-packages/ledgers. No tenant/customer identifiers, payloads, credentials, destinations, or financial values.';
